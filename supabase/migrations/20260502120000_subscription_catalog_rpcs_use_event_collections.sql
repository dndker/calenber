-- 구독 카탈로그 RPC가 과거 본문(public.event_categories 조인)을 유지한 채
-- 테이블만 event_collections로 바뀐 DB에서 42P01이 나는 경우를 방지한다.
-- 항상 public.event_collections만 참조하도록 동일 시그니처로 재정의한다.
